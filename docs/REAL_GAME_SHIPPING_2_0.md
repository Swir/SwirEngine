...
The gate treats the maintained source-only 2D, 3D and multiplayer projects as engine integration fixtures rather than decorative demos. Each fixture is copied into an isolated project, given production scene/content/input/settings/save configuration, validated, staged with `ProjectExporter`, and—when runtime validation is enabled—executed both from source and from the staged output.
...
- deterministic project, scene, content, input/settings, game-state and export-manifest evidence;
...
The gate intentionally reuses the existing project manifest, scene package, content graph, shipping defaults, save/profile and export systems instead of creating a parallel demo-only shipping abstraction.

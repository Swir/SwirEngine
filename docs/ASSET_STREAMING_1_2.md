...
- keep resident-byte accounting O(1) so diagnostics and eviction checks do not rescan the complete residency set every frame.
...
Resident-byte totals are maintained incrementally on admission and eviction. This makes `resident_bytes`, diagnostics and byte-budget checks constant-time with respect to the number of resident assets instead of repeatedly summing the entire LRU set.

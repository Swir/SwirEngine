...
class TransientRenderResourcePool(Generic[ResourceT]):
    ...
    Handles are generation checked, so a stale lease cannot release a resource that has since been
    reacquired by another caller.
    """
    ...
    def acquire(self, descriptor: RenderResourceDescriptor) -> ResourceLease[ResourceT]:
        ...
        if reusable is not None:
            ...
            return ResourceLease(
                handle=RenderResourceHandle(reusable, entry.generation),
        ...
        return ResourceLease(
            handle=RenderResourceHandle(slot, 1),
            descriptor=descriptor,
            resource=resource,
            reused=False,
        )
    ...
    def trim(self, *, target_resources: int = 0, target_bytes: int = 0) -> int:
        ...
        removed = 0
        while (
            len(self._entries) > target_resources or self._resident_bytes > target_bytes
        ):
    ...
    def _validate_leased_handle(self, handle: RenderResourceHandle) -> _Entry[ResourceT]:
        ...
        if entry is None or not entry.leased or entry.generation != handle.generation:
...
class RenderPlanPoolSession(Generic[ResourceT]):
    ...
    def abort(self) -> None:
        for name in sorted(tuple(self._leases)):
            lease = self._leases.pop(name)
            try:
                self.pool.release(lease.handle)

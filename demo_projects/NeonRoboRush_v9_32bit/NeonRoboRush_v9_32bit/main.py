...
class Enemy:
    ...
    phase: float = 0.0
    @property
    def all_frames(self): return self.frames_r+self.frames_l
...
class NeonRoboRush32:
    def __init__(self):
        self.game=Game(
            "Neon Robo Rush v9 — 32-Bit Edition",
            W,H,mode="2d",target_fps=144,fixed_hz=FIXED_HZ,asset_root=ASSET_DIR
        )
    ...
    def _background(self):
        self.sky=self.rect(0,0,1600,900,Color(.02,.03,.07,1),layer=-240)
        self.bg_far=[];self.bg_city=[];self.bg_front=[]
        for i in range(4):
            self.bg_far.append(self.game.sprite("bg_far.png",x=i*1600,y=0,width=1600,height=720,layer=-230))
            self.bg_city.append(self.game.sprite("bg_city.png",x=i*1600,y=0,width=1600,height=720,layer=-220))
            self.bg_front.append(self.game.sprite("bg_front.png",x=i*1600,y=0,width=1600,height=720,layer=-215))

    def _level(self):
        self.solids=[]
        for x,y,w,h,style in SOLIDS:
            self.solids.append(Solid(x,y,w,h))
            self.game.sprite(f"platform{style}.png",x=x,y=y,width=w,height=max(80,h+20),layer=-2)

        self.game.sprite("checkpoint.png",x=CHECKPOINT_X+16,y=-94,width=96,height=134,layer=5)
        self.checkpoint_glow=self.rect(CHECKPOINT_X+10,-70,28,28,Color(.18,.48,.65,.22),rotation=45,layer=4)

        for x in (460,1260,2060,2860,3660):
            self.rect(x,-135,52,132,Color(.04,.12,.18,1),layer=2)
            self.rect(x,-88,64,9,Color(.10,.78,.88,.65),layer=3)

    def _hero(self):
        self.hero_shadow=self.game.sprite("shadow.png",x=self.px,y=self.py-44,width=88,height=30,layer=8)
        self.hero_frames={}
        specs={
            "idle_r":[f"hero_idle{i}_r.png" for i in range(3)],
            "idle_l":[f"hero_idle{i}_l.png" for i in range(3)],
            "run_r":[f"hero_run{i}_r.png" for i in range(6)],
            "run_l":[f"hero_run{i}_l.png" for i in range(6)],
        ...
        for state,names in specs.items():
            arr=[]
            for name in names:
                arr.append(self.game.sprite(name,x=self.px,y=self.py+18,width=126,height=126,visible=False,layer=20))
            self.hero_frames[state]=arr

    def _enemies(self):
        self.enemies=[]
        for x,y,patrol,speed in ENEMY_SPAWNS:
            rr=[self.game.sprite(f"enemy{i}_r.png",x=x,y=GROUND_TOP+48,width=96,height=96,visible=False,layer=15) for i in range(4)]
            ll=[self.game.sprite(f"enemy{i}_l.png",x=x,y=GROUND_TOP+48,width=96,height=96,visible=False,layer=15) for i in range(4)]
            sh=self.game.sprite("shadow.png",x=x,y=GROUND_TOP-6,width=60,height=22,layer=12)
            self.enemies.append(Enemy(rr,ll,sh,x,y,x,patrol,speed))

    def _cells(self):
        self.cells=[]
        for x,y in CELLS:
            glow=self.rect(x,y,52,52,Color(.14,1,.67,.12),rotation=45,layer=4)
            spr=self.game.sprite("power_cell.png",x=x,y=y,width=48,height=62,layer=6)
            self.cells.append(Cell(spr,glow))

    def _portal(self):
        self.portal_glow=self.rect(GOAL_X,-126,96,132,Color(.18,1,.68,.06),layer=3)
        self.portal=self.game.sprite("portal.png",x=GOAL_X,y=-118,width=122,height=180,layer=6)
    ...
    def _ui(self):
        self.hud_panel=self.rect(-430,310,350,56,Color(.015,.035,.07,.78),screen_space=True,layer=90)
        self.obj_panel=self.rect(300,310,510,56,Color(.015,.035,.07,.74),screen_space=True,layer=90)
    ...
    # -------- collision --------
    @staticmethod
    def _overlap_1d(a0,a1,b0,b1): return a1>b0 and a0<b1
    ...
    def _move_x(self,dt):
        ...
        for s in self.solids:
            ...
            self.vx=0;l,r,b,t=self._player_box(nx,self.py)
        self.px=nx
    ...
    def _move_y(self,dt):
        ...
        if dy<=0:
            ...
            for s in self.solids:
                ...
                if ob>=s.top-.01 and nb<=s.top:best=s.top if best is None else max(best,s.top)
            if best is not None:ny=best+PLAYER_H/2;self.vy=0;self.grounded=True
        else:
            best=None
            for s in self.solids:
                if not self._overlap_1d(nl,nr,s.left,s.right):continue
                if ot<=s.bottom+.01 and nt>=s.bottom:best=s.bottom if best is None else min(best,s.bottom)
            if best is not None:ny=best-PLAYER_H/2;self.vy=0
        self.py=ny
    ...
    def _spawn_particles(self,x,y,count,color,sx,sy,spread=1.0,size=6):
        ...
        for p in self.particles:
            ...
            p.max_life=p.life=.32+abs(a)*.14;p.size=size;p.color=color;p.node.visible=True
            emitted+=1
            if emitted>=count:break

    def _update_particles(self,dt):
        for p in self.particles:
            if p.life<=0:p.node.visible=False;continue
            p.life-=dt
            if p.life<=0:p.node.visible=False;continue
            p.x+=p.vx*dt;p.y+=p.vy*dt;p.vy-=450*dt
            t=p.life/p.max_life
            p.node.x=p.x;p.node.y=p.y;p.node.width=p.node.height=p.size*(.75+.25*t)
            p.node.color=Color(p.color.r,p.color.g,p.color.b,max(0,min(1,t*.9)));p.node.rotation+=dt*180

    # -------- gameplay --------
    def _fixed(self,dt):
        if self.won or self.game_over:return
        left=self.game.key("A") or self.game.key("LEFT");right=self.game.key("D") or self.game.key("RIGHT")
        move=float(right)-float(left)
        if move<0:self.facing=-1
        elif move>0:self.facing=1
        target=move*RUN_SPEED;accel=GROUND_ACCEL if self.grounded else AIR_ACCEL
        if move:
            delta=target-self.vx;step=accel*dt;self.vx+=max(-step,min(step,delta))
        else:
            step=FRICTION*dt;self.vx=0 if abs(self.vx)<=step else self.vx-math.copysign(step,self.vx)
        self.vy=max(MAX_FALL,self.vy+GRAVITY*dt)
        self._move_x(dt);self._move_y(dt)
        if self.grounded and not self.last_grounded:self._spawn_particles(self.px,self.py-PLAYER_H/2+2,4,Color(.7,.95,1,1),0,80,1.4,6)
        if self.py<-470:self._hurt()

    def _hurt(self):
        if self.invulnerable>0 or self.won or self.game_over:return
        self.health-=1;self._sound("hit.wav",.22);self._spawn_particles(self.px,self.py+8,6,Color(1,.32,.32,1),0,110,1.8,7)
        if self.health<=0:self.game_over=True;self.vx=self.vy=0;return
    ...
    def _hero_state(self):
        ...
        if self.invulnerable>0 and int(self.invulnerable*12)%2==0:return f"hit_{side}",0
        if not self.grounded:return (f"jump_{side}",0) if self.vy>=0 else (f"fall_{side}",0)
        if abs(self.vx)>25:return f"run_{side}",int(self.hero_anim_clock*13)%6
        return f"idle_{side}",int(self.hero_anim_clock*2.2)%3
    ...
    def _animate_hero(self,dt):
        ...
        s=self.hero_frames[state][idx]
        s.visible=True;s.x=self.px;s.y=self.py+18;s.rotation=(-2*self.facing if "run" in state else 0)
        self.hero_shadow.x=self.px;self.hero_shadow.y=GROUND_TOP-6 if self.py<0 else self.py-46
        self.hero_shadow.width=88 if self.grounded else max(40,88-abs(self.vy)*.036)

    def _enemy_update(self,dt):
        for e in self.enemies:
            if not e.alive:continue
            e.phase+=dt*5;e.x+=e.direction*e.speed*dt
            if e.x>e.origin+e.patrol or e.x<e.origin-e.patrol:e.direction*=-1
            frames=e.frames_r if e.direction>0 else e.frames_l;idx=int(e.phase*1.8)%4
            for s in e.all_frames:s.visible=False
            frames[idx].visible=True;frames[idx].x=e.x;frames[idx].y=GROUND_TOP+49+abs(math.sin(e.phase))*2
            e.shadow.x=e.x;e.shadow.y=GROUND_TOP-6

        pl,pr,pb,pt=self._player_box()
        for e in self.enemies:
            if not e.alive:continue
            ex=e.x;ey=GROUND_TOP+18;el,er,eb,et=ex-19,ex+19,ey-19,ey+19
    ...
    def _cell_update(self,dt):
        for i,(c,(x,y)) in enumerate(zip(self.cells,CELLS)):
            ...
            bob=math.sin(self.time*4+i)*5;c.sprite.x=x;c.sprite.y=y+bob;c.sprite.rotation+=dt*12
            c.glow.x=x;c.glow.y=y+bob;c.glow.rotation+=dt*46
            if abs(self.px-x)<30 and abs(self.py-y)<43:
                c.active=False;c.sprite.visible=False;c.glow.visible=False;self.energy+=1
                self._sound("pickup.wav",.18);self._spawn_particles(x,y+8,5,Color(.4,1,.78,1),0,95,1.4,6)
        if not self.checkpoint_active and self.px>=CHECKPOINT_X:
            self.checkpoint_active=True;self.checkpoint=(CHECKPOINT_X,SPAWN_Y);self._sound("checkpoint.wav",.18)
        ...
        if self.energy==self.total_energy and abs(self.px-GOAL_X)<50:
            self.won=True;self.vx=self.vy=0;self._sound("win.wav",.2);self._spawn_particles(GOAL_X,-100,10,Color(.3,1,.8,1),0,140,2.2,7)

    # -------- camera / presentation --------
    def _camera_update(self):
        cam=self.game.camera;screen_x=self.px-cam.x
        ...
        far0=cam.x*.14-800;city0=cam.x*.31-800;front0=cam.x*.56-800
        for i,s in enumerate(self.bg_far):s.x=far0+i*1600
        for i,s in enumerate(self.bg_city):s.x=city0+i*1600
        for i,s in enumerate(self.bg_front):s.x=front0+i*1600
        self.sky.x=cam.x

    def _portal_update(self,dt):
        unlocked=self.energy==self.total_energy
        self.portal.rotation=math.sin(self.time*1.7)*.8
        self.portal_glow.color=Color(.18,1,.68,.14 if unlocked else .045)
        self.checkpoint_glow.rotation+=dt*55
        self.checkpoint_glow.color=Color(.22,1,.76,.40) if self.checkpoint_active else Color(.18,.48,.65,.22)

    def _ui_update(self):
        hearts="♥"*self.health+"·"*(3-self.health)
        self.hud.text=f"ROBO  {hearts}    ENERGY {self.energy}/{self.total_energy}"
        if self.won:
            self.objective.text="SECTOR CLEARED";self.banner.text="LEVEL COMPLETE";self.subbanner.text="NEON ROBO RUSH — 32-BIT EDITION"
        elif self.game_over:
    ...
    def _update(self,dt):
        ...
        if not self.won and not self.game_over:
            ...
            if self.game.key_pressed("SPACE") or self.game.key_pressed("W") or self.game.key_pressed("UP"):self.jump_buffer=.12
            else:self.jump_buffer=max(0,self.jump_buffer-dt)
            if self.jump_buffer>0 and self.coyote>0:
                self.vy=JUMP_SPEED;self.grounded=False;self.jump_buffer=0;self.coyote=0;self._sound("jump.wav",.18)
                self._spawn_particles(self.px,self.py-PLAYER_H/2+6,4,Color(.55,.92,1,1),0,110,1.5,6)
            if (self.game.key_released("SPACE") or self.game.key_released("W") or self.game.key_released("UP")) and self.vy>250:self.vy*=.56

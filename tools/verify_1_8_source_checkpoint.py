"""Audit the SwirEngine 1.8 source-only checkpoint contract.

The auditor deliberately separates development validation from final closeout:
without ``--require-complete`` a 9/10 candidate may be validated while Milestone 10
is being built; strict mode refuses to pass until the authoritative roadmap is
exactly 10/10 and 100.0%.
"""

from __future__ import annotations

import argparse
import re
from dataclasses import dataclass
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
ROADMAP = ROOT / "ROADMAP_1_8.md"
PYPROJECT = ROOT / "pyproject.toml"
WORKFLOWS = ROOT / ".github" / "workflows"

STABLE_PUBLIC_VERSION = "1.5.0"
EXPECTED_MILESTONES = 10

MILESTONE_RE = re.compile(r"^- \[([ xX])\] \*\*(\d+)\.", re.MULTILINE)
PROGRESS_RE = re.compile(
    r"Current verified progress:\s*(\d+)\s*/\s*(\d+)\s*milestones\s*=\s*([0-9]+(?:\.[0-9]+)?)%",
    re.IGNORECASE,
)
VERSION_RE = re.compile(r'^version\s*=\s*"([^"]+)"\s*$', re.MULTILINE)

REQUIRED_18_FILES = (
    "src/swirengine/render_graph18.py",
    "src/swirengine/render_resources18.py",
    "src/swirengine/render_uploads18.py",
    "src/swirengine/render_submission18.py",
    "src/swirengine/visibility18.py",
    "src/swirengine/render_timing18.py",
    "src/swirengine/render_quality18.py",
    "src/swirengine/renderer2_bridge18.py",
    "src/swirengine/render_showcase18.py",
    "tests/test_render_showcase_1_8.py",
    "examples/demo_render_showcase_1_8.py",
    "tools/benchmark_render_showcase_1_8.py",
    "docs/RENDER_SHOWCASE_1_8.md",
    ".github/workflows/render-showcase-1-8.yml",
    "tools/verify_1_8_source_checkpoint.py",
    "tests/test_source_checkpoint_1_8.py",
    "docs/RELEASE_HARDENING_1_8.md",
    ".github/workflows/source-checkpoint-1-8.yml",
    "CHANGELOG.d/1.8-source-checkpoint.md",
)

REQUIRED_18_WORKFLOWS = (
    "render-graph-1-8.yml",
    "transient-render-resources-1-8.yml",
    "texture-upload-residency-1-8.yml",
    "material-pipeline-cache-1-8.yml",
    "visibility-lod-1-8.yml",
    "gpu-timing-capture-1-8.yml",
    "dynamic-quality-1-8.yml",
    "renderer2-bridge-1-8.yml",
    "render-showcase-1-8.yml",
    "source-checkpoint-1-8.yml",
)

LOCKED_COMPATIBILITY_FILES = (
    "ROADMAP_1_4.md",
    "ROADMAP_1_5.md",
    "ROADMAP_1_6.md",
    "ROADMAP_1_7.md",
    "tools/verify_1_6_source_checkpoint.py",
    "tools/verify_1_7_source_checkpoint.py",
    ".github/workflows/source-checkpoint-1-6.yml",
    ".github/workflows/source-checkpoint-1-7.yml",
)

FORBIDDEN_18_WORKFLOW_PATTERNS = (
    re.compile(r"(?:release|publish|tag).*1[-_.]?8", re.IGNORECASE),
    re.compile(r"1[-_.]?8.*(?:release|publish|tag)", re.IGNORECASE),
)


class CheckpointError(RuntimeError):
    """Raised when the 1.8 source checkpoint contract is violated."""


@dataclass(frozen=True)
class RoadmapState:
    completed: int
    total: int
    declared_completed: int
    declared_total: int
    declared_percent: float

    @property
    def calculated_percent(self) -> float:
        return (self.completed / self.total) * 100.0 if self.total else 0.0


def _text(path: Path) -> str:
    try:
        return path.read_text(encoding="utf-8")
    except FileNotFoundError as exc:
        raise CheckpointError(f"required file is missing: {path.relative_to(ROOT)}") from exc


def parse_roadmap(text: str) -> RoadmapState:
    milestones = MILESTONE_RE.findall(text)
    if len(milestones) != EXPECTED_MILESTONES:
        raise CheckpointError(
            f"ROADMAP_1_8.md must contain exactly {EXPECTED_MILESTONES} numbered milestones; "
            f"found {len(milestones)}"
        )

    numbers = [int(number) for _, number in milestones]
    expected_numbers = list(range(1, EXPECTED_MILESTONES + 1))
    if numbers != expected_numbers:
        raise CheckpointError(
            f"ROADMAP_1_8.md milestone numbering must be {expected_numbers}; found {numbers}"
        )

    completed = sum(mark.lower() == "x" for mark, _ in milestones)
    match = PROGRESS_RE.search(text)
    if match is None:
        raise CheckpointError("ROADMAP_1_8.md is missing the verified progress declaration")

    state = RoadmapState(
        completed=completed,
        total=len(milestones),
        declared_completed=int(match.group(1)),
        declared_total=int(match.group(2)),
        declared_percent=float(match.group(3)),
    )
    if state.declared_completed != state.completed or state.declared_total != state.total:
        raise CheckpointError(
            "ROADMAP_1_8.md progress declaration does not match its milestone checkboxes: "
            f"declared {state.declared_completed}/{state.declared_total}, "
            f"calculated {state.completed}/{state.total}"
        )
    if abs(state.declared_percent - state.calculated_percent) > 0.05:
        raise CheckpointError(
            "ROADMAP_1_8.md percentage does not match its milestone checkboxes: "
            f"declared {state.declared_percent:.1f}%, "
            f"calculated {state.calculated_percent:.1f}%"
        )
    return state


def _require_files(paths: tuple[str, ...], *, label: str) -> None:
    missing = [path for path in paths if not (ROOT / path).is_file()]
    if missing:
        joined = ", ".join(missing)
        raise CheckpointError(f"missing {label}: {joined}")


def _verify_public_version() -> None:
    pyproject = _text(PYPROJECT)
    match = VERSION_RE.search(pyproject)
    if match is None:
        raise CheckpointError("pyproject.toml does not expose a static project version")
    version = match.group(1)
    if version != STABLE_PUBLIC_VERSION:
        raise CheckpointError(
            "source-only 1.8 development must keep the public package frozen at "
            f"{STABLE_PUBLIC_VERSION}; found {version}"
        )


def _verify_release_freeze(roadmap_text: str) -> None:
    lowered = roadmap_text.lower()
    if "next public github release and pypi publication remains swirengine 2.0 only" not in lowered:
        raise CheckpointError("ROADMAP_1_8.md must preserve the 2.0-only public release policy")
    if "do not create `v1.8.0`, a github release, or a pypi publish" not in lowered:
        raise CheckpointError("ROADMAP_1_8.md must explicitly forbid an intermediate 1.8 release")

    forbidden = []
    for path in WORKFLOWS.glob("*.y*ml"):
        if any(pattern.search(path.name) for pattern in FORBIDDEN_18_WORKFLOW_PATTERNS):
            forbidden.append(path.name)
    if forbidden:
        raise CheckpointError(
            "intermediate 1.8 release/tag/publish workflows are forbidden: " + ", ".join(sorted(forbidden))
        )


def _verify_workflows() -> None:
    missing = [name for name in REQUIRED_18_WORKFLOWS if not (WORKFLOWS / name).is_file()]
    if missing:
        raise CheckpointError("missing 1.8 validation workflows: " + ", ".join(missing))

    checkpoint = _text(WORKFLOWS / "source-checkpoint-1-8.yml")
    required_tokens = (
        "3.10",
        "3.13",
        "3.14",
        "verify_1_8_source_checkpoint.py",
        "verify_1_7_source_checkpoint.py",
        "python -m build",
        "twine check",
        "demo_render_showcase_1_8.py",
    )
    missing_tokens = [token for token in required_tokens if token not in checkpoint]
    if missing_tokens:
        raise CheckpointError(
            "source-checkpoint-1-8.yml is missing required validation contracts: "
            + ", ".join(missing_tokens)
        )
    lowered = checkpoint.lower()
    if "pypi" in lowered or "gh release" in lowered or "twine upload" in lowered:
        raise CheckpointError("source-checkpoint-1-8.yml must validate only and never publish")


def audit(*, require_complete: bool = False) -> RoadmapState:
    roadmap_text = _text(ROADMAP)
    state = parse_roadmap(roadmap_text)

    _require_files(REQUIRED_18_FILES, label="1.8 checkpoint files")
    _require_files(LOCKED_COMPATIBILITY_FILES, label="locked compatibility files")
    _verify_public_version()
    _verify_release_freeze(roadmap_text)
    _verify_workflows()

    hardening = _text(ROOT / "docs" / "RELEASE_HARDENING_1_8.md").lower()
    for token in ("source-only", "1.5.0", "python 3.10", "python 3.13", "python 3.14", "2.0"):
        if token not in hardening:
            raise CheckpointError(f"RELEASE_HARDENING_1_8.md is missing required contract: {token}")

    if require_complete and (
        state.completed != EXPECTED_MILESTONES
        or state.total != EXPECTED_MILESTONES
        or abs(state.declared_percent - 100.0) > 0.05
    ):
        raise CheckpointError(
            "strict 1.8 source checkpoint refuses completion below 10/10 = 100.0%; "
            f"current roadmap is {state.completed}/{state.total} = {state.declared_percent:.1f}%"
        )
    return state


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--require-complete",
        action="store_true",
        help="refuse to pass until ROADMAP_1_8.md is exactly 10/10 = 100.0%%",
    )
    args = parser.parse_args()

    try:
        state = audit(require_complete=args.require_complete)
    except CheckpointError as exc:
        print(f"source-checkpoint-1.8: FAIL: {exc}")
        return 1

    mode = "strict-complete" if args.require_complete else "development"
    print(
        "source-checkpoint-1.8: PASS "
        f"mode={mode} roadmap={state.completed}/{state.total} "
        f"progress={state.declared_percent:.1f}% public-version={STABLE_PUBLIC_VERSION}"
    )
    print("Release/PyPI: frozen until SwirEngine 2.0")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())

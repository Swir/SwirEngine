...
def test_gltf_primitives_preserve_material_and_uvs(tmp_path):
    ...
    assert asset.material.metallic == pytest.approx(0.2)
    assert asset.material.roughness == pytest.approx(0.7)
    assert asset.material.pbr_enabled is True
    assert asset.material.diffuse == pytest.approx(0.6)
    assert asset.material.specular == pytest.approx(0.232)

...
def test_recovery_reuses_registered_release_workflow_identity() -> None:
    workflow = _workflow()
    trigger = workflow.split("jobs:", 1)[0]

    assert "name: Release" in trigger
    assert "workflow_dispatch:" in trigger
    assert "branches:\n      - main" in trigger
    assert 'paths:\n      - ".github/workflows/release.yml"' in trigger
    assert "environment: pypi" in workflow
    assert "id-token: write" in workflow
    assert "pypa/gh-action-pypi-publish@release/v1" in workflow


def test_recovery_is_pinned_to_immutable_verified_2_0_source() -> None:
    workflow = _workflow()

    assert "RELEASE_TAG: v2.0.0" in workflow
...
def test_recovery_fails_closed_and_never_moves_or_overwrites_public_history() -> None:
    workflow = _workflow()

    assert "https://pypi.org/pypi/swirengine/2.0.0/json" in workflow
    assert "gh release view" in workflow
    assert "Inconsistent public 2.0.0 state" in workflow
    assert "Refusing to overwrite or guess recovery semantics" in workflow
...
def test_recovery_revalidates_real_game_packaging_and_public_installation() -> None:
    workflow = _workflow()
